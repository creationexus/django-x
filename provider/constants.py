...
EXPIRE_DELTA = getattr(settings,'OAUTH_EXPIRE_DELTA',timedelta(days=150))#timedelta(days=365))
# Expiry delta for public clients (which typically have shorter lived tokens)
EXPIRE_DELTA_PUBLIC = getattr(settings,'OAUTH_EXPIRE_DELTA_PUBLIC',timedelta(days=137))#timedelta(days=30))
EXPIRE_CODE_DELTA = getattr(settings,'OAUTH_EXPIRE_CODE_DELTA',timedelta(seconds=1740*60))
...
ENFORCE_SECURE = getattr(settings,'OAUTH_ENFORCE_SECURE',True)
