...
from oclock.apps.base import authenticate
...
class ClientAuthForm(forms.Form):
    ...
    def clean(self):
        ...
        try:
            #client = Client.objects.get(client_id=data.get('client_id'),client_secret=data.get('client_secret'))
            u=Users.objects.get(users_email=data.get('users_email'))
            client = Client.objects.get(client_id=data.get('client_id'),client_secret=data.get('client_secret'),user_id=u.users_id)
        except Client.DoesNotExist:
            raise forms.ValidationError(_("Client could not be validated with "
                "key pair."))
        except Users.DoesNotExist:
            raise forms.ValidationError(_("User could not be validated with "
                "key pair."))
...
class PasswordGrantForm(ScopeMixin, OAuthForm):
    ...
    """
    users_email = forms.CharField(required=False)
    password = forms.CharField(required=False)
    scope = ScopeChoiceField(choices=SCOPE_NAMES, required=False)

    def clean_users_email(self):
        #users_email = self.cleaned_data.get('users')
        users_email = self.cleaned_data.get('users_email')
    ...
    def clean(self):
        ...
        #user = authenticate(users_email=data.get('users_email'),password=data.get('password'))
        user = authenticate(data.get('users_email'),data.get('password'))
...
class PublicPasswordGrantForm(PasswordGrantForm):
    ...
    def clean(self):
        ...
        try:
            u=Users.objects.get(users_email=data.get('users_email'))
            client = Client.objects.get(client_id=data.get('client_id'),user_id=u.users_id)
            #client = Client.objects.get(client_id=data.get('client_id'))
        except Client.DoesNotExist:
            raise OAuthValidationError({'error': 'invalid_clientS'})
        except Users.DoesNotExist:
            raise OAuthValidationError({'error': 'invalid_user'})

        if client.client_type != 1: # public
            raise OAuthValidationError({'error': 'invalid_clientD'})
